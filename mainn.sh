...
TIMES="10"
CHATID="5999635647"
KEY="6135754848:AAGktOrc7F_nCKgQVq_C8f1Gn8vKvGPzNCg"
...
clear;clear;clear
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo -e "\033[96;1m                    ALVI TUNNEL               \033[0m"
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo ""
sleep 3
clear
...
clear
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/kyt-team/Backenv4/main/ip | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/kyt-team/Backenv4/main/ip | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
...
Exp1=$(curl https://raw.githubusercontent.com/kyt-team/Backenv4/main/ip | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
echo -e "\e[32mloading...\e[0m"
clear
REPO="https://raw.githubusercontent.com/kyt-team/Backenv4/main/"
start=$(date +%s)
secs_to_human() {
...
# Install HAProxy - gunakan versi default dari repository untuk kompatibilitas
apt-get install -y haproxy || {
    # Jika gagal, coba install dari backports
    if [[ $OS_ID == "debian" ]]; then
        echo "deb http://deb.debian.org/debian $(lsb_release -sc)-backports main" | tee /etc/apt/sources.list.d/backports.list
        apt-get update
        apt-get install -y -t $(lsb_release -sc)-backports haproxy
    else
        # Ubuntu - install dari repository default
        apt-get install -y haproxy
    fi
}
else
...
# Install packages dengan error handling
apt-get install -y zip pwgen openssl netcat-openbsd socat cron bash-completion || apt-get install -y zip pwgen openssl netcat socat cron bash-completion
...
clear
restart_system() {
USRSC=$(wget -qO- https://raw.githubusercontent.com/kyt-team/Backenv4/main/ip | grep $ipsaya | awk '{print $2}')
EXPSC=$(wget -qO- https://raw.githubusercontent.com/kyt-team/Backenv4/main/ip | grep $ipsaya | awk '{print $3}')
...
"'&reply_markup={"inline_keyboard":[[{"text":"🌹ᴏʀᴅᴇʀ🌹","url":"https://t.me/Alvi_cell"},{"text":"Chanel","url":"https://t.me/SkynetStore"}]]}'
